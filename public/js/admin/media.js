			...
			this.init = function(){
				...
				files.on("dblclick", "li .file_link", function(){
					...
					if (imageMimeTypes.indexOf(type) > -1) {
						...
						//tinyMCE.windowManager.close(this);
												
						//var ed = parent.tinymce.editors[0];
						//ed.windowManager.windows[1].close();
						//win.tinyMCE.activeEditor.windowManager.windows[1].close();
						
						//$('.mce-btn.mce-open').parent().find('.mce-textbox').val(manager.selected_file.path);
				...
				function setCurrentSelected(cur){
					...
					if (type != 'folder'){
						...
						filePath.val(manager.selected_file.path);
						var imageMimeTypes = ['image/jpeg', 'image/png', 'image/gif'];
						...
						filePath.attr('file-type', type);

						//console.log(type);
						
				
						//$(targetInput.val()).val(manager.selected_file.path);
					}
				}
