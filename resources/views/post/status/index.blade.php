...
@section('page-title', __('Post status'))
...
@section('content')
    ...
    <div class="row">
        <div class="col-6">
            <form class="form-inline" action="{{ route('post-status.index') }}" method="GET">
        ...
        <div class="col-6 d-flex justify-content-end">
            <a role="button" class="btn btn-primary" href="{{ route('post-status.create') }}"><i
    ...
    <table class="table table-hover status-table">
        ...
        @foreach($list as $item)
            <tr>
                ...
                <td>
                    <a href="{{ route('post-status.edit', ['id' => $item->id]) }}" class="btn btn-sm btn-primary">
                        <i class="fas fa-edit"></i>
                    </a>
                    <a href="{{ route('post-status.delete', ["id"=>$item->id]) }}"
                       class="btn btn-sm btn-danger ml-1 {{ $item->count_post > 0 ? 'disabled' : '' }}"
                       title="@lang('លុបស្ថានភាព')"
                       data-toggle="tooltip"
                       data-placement="top"
                       data-method="DELETE"
