...
@section('content')
    ...
    <div class="create-post">
        <form action="{{ route('post.update') }}" method="POST" id="post-form" enctype="multipart/form-data">
            @csrf
            <input type="hidden" name="id" value="{{ $post->id }}">
            <input type="hidden" name="old_status" value="{{ $post->status }}">
            <input type="hidden" name="old_category_id" value="{{ $post->category_id }}">
            <input type="hidden" name="old_title" value="{{ $post->title }}">
            <input type="hidden" name="old_image" value="{{ $post->image }}">
            <input type="hidden" name="old_image" value="{{ $post->image }}">
            <input type="hidden" name="old_body" value="{{ $post->body }}">
            <div class="row">
                <div class="col-9">
                ...
                <div class="col-3">
                    ...
                    <div class="form-group">
                        <label for="is_popular">@lang('Is Popular')</label>
                        <div class="form-check">
                            <input class="form-check-input" type="radio" name="is_popular" id="is_popular" value="1" {{ $post->is_popular ? 'checked' : '' }}>
                            <label class="form-check-label" for="is_popular">
                                បង្ហាញ
                            </label>
                        </div>
                        <div class="form-check">
                            <input class="form-check-input" type="radio" name="is_popular" id="is_popular" value="0" {{ !$post->is_popular ? 'checked' : '' }}>
                            <label class="form-check-label" for="is_popular">
                                មិនបង្ហាញ
                            </label>
                        </div>
                    ...
                    <div class="form-group">
                        ...
                        <select name="status" class="form-control select-box" id="status">
                            @foreach($postStatus as $status)
                                <option
                                    value="{{ $status->slug }}" {{ $status->slug === $post->status ? 'selected' : '' }}>{{ $status->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="form-group text-center mt-4 {{ $post->image ? '' : 'd-none' }}">
                        <img id="image_preview" class="image_preview" src="{{ getFileCDN($post->image) }}" alt="image"
                             width="300">
                    </div>
                    <div class="form-group">
