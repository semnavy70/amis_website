...
@section('page-title', __('Advertise'))
...
@section('content')
    ...
    <div class="row">
        <div class="col-6">
            <form class="form-inline" action="{{ route('advertise.index') }}" method="GET">
        ...
        <div class="col-6 d-flex justify-content-end">
            <a role="button" class="btn btn-primary" href="{{ route('advertise.create') }}">
                <i class="fas fa-plus mr-1"></i>@lang('Create new')</a>
        </div>
    </div>
    <table class="table table-hover advertise-table">
        ...
        @foreach($list as $item)
            <tr>
                ...
                <td>{{ $item->name }}</td>
                <td><img src="{{ getFileCDN($item->image) }}" width="140" alt=""></td>
                <td>{{ $item->page_name }}</td>
                <td>{{ $item->blog_name }}</td>
                <td>{{ $item->order }}</td>
                <td class="text-gray-500">{{ $item->is_active ? 'បង្ហាញ' : 'មិនបង្ហាញ' }}</td>
                <td>{{ $item->click_count }}</td>
                <td>
                    <a href="{{ route('advertise.edit', ["id"=>$item->id]) }}" class="btn btn-sm btn-primary">
                        <i class="fas fa-edit"></i>
                    </a>
                    <a href="{{ route('advertise.delete', ["id"=>$item->id]) }}"
