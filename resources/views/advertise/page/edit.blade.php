...
@section('content')
    ...
    <form action="{{route("advertise-page.update")}}" method="POST" id="advertise-page-form">
        @csrf
        <input type="hidden" name="id" value="{{$advertisePage->id}}">
        <div class="form-group">
            <label for="name">@lang('Name*')</label>
            <input type="text" class="form-control" id="name" name="name" value="{{old("name")??$advertisePage->name}}">
        </div>
        <div class="form-group">
            <label for="slug">@lang('Slug*')</label>
            <input type="text" class="form-control" id="slug" name="slug" disabled
                   value="{{old("slug")??$advertisePage->slug}}">
        </div>
        <div class="form-group">
            <label for="order">@lang('Order')</label>
            <input type="number" class="form-control" id="order" name="order"
                   value="{{old("order")??$advertisePage->order}}">
        </div>
        <div class="row mb-3">
            <div class="col-8"></div>
